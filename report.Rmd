...
- High Speed Network **100 Gbit InfiniBand**: with $peak$ $perf_{theo}$ of 100 Gbit/s = 12.5 GB/s = **12500 MB/s**;  $(1)$
- In band management network **25 Gbit Ethernet**: with $peak$ $perf_{theo}$ of 25 Gbit/s = 3.125 GB/s = **3125 MB/s**;   $(2)$
...
It reports the time $\Delta t$ (in $\mu s$), throughput $\frac{X}{\Delta t}$ (in MB/s), number of repetitions and the message size $X$ (in B) in the standard output:
...
**Each specific run is repeated 10 times** with `-msglog 28` and each time the entire node was reserved in order to reduce noise in measurements. Thus, from the measurements, it was taken the mean time $\Delta t$ and the mean throughput $X/\Delta t$ between the 10 results for each message size $X$, along with some statistics (ex. maximum error on time and on throughput $e_t = \frac{\Delta t_{max} - \Delta t_{min}}{2}$).
...
* **Intel**: version `intel` available on ORFEO.
