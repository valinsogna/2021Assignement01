...
- High Speed Network **100 Gbit InfiniBand**: with $pick$ $perf_{theo}$ of 100 Gbit/s = 12.5 GB/s = 12500 MB/s;  $(1)$
- In band management network **25 Gbit Ethernet**: with $pick$ $perf_{theo}$ of 25 Gbit/s = 3.125 GB/s = 3125 MB/s;   $(2)$
...
**Each specific run is repeated 10 times** with `-msglog 28` and each time the entire node was reserved in order to reduce noise in measurements. Thus, from the measurements, it was taken the mean time $\Delta t$ between the 10 results for each message size $X$, along with some statistics (ex. maximum error on time $e_t = \frac{\Delta t_{max} - \Delta t_{min}}{2}$).
